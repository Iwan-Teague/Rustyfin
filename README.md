...
- AI
  ...
  - `/api/v1/ai/chat` now returns deterministic server-authored answers for explicit current-date/current-time questions through `system_get_current_datetime`, including named-place local-time lookups resolved through public geocoding/timezone data, deterministic next-event answers through grounded calendar ordering, concrete birthday detail lists from grounded birthday payloads, and deterministic Rustyfin network connect guidance from grounded topology payloads so LAN/IP/port prompts do not drift into hallucinated locations or endpoints; local-network answers must prefer the real LAN interface over Docker bridges, virtual adapters, or Tailscale overlay addresses
