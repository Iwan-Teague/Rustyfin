...
- AI
  ...
  - `/ai` conversation history now supports persisted per-user grouping plus manual within-group ordering, so folder-style thread organization survives reloads and redeploys instead of relying on `updated_ts` sorting alone
  - `/api/v1/ai/chat` now uses a model-assisted structured planner for grounded tool selection, with typed planner validation/repair, strict registry/role validation, planner diagnostics, and deterministic fallback plus deterministic entity-follow-up resolution
  - `/ai` conversation threads now persist compact conversation memory for older turns, including a summary plus durable facts, user preferences, open loops, and active topics, so long chats do not rely only on replaying the raw transcript
  - accepted conversation-backed turns are now durably journaled before first-token generation, and long-thread compactions now write explicit compact-boundary records so prompt-memory recovery survives restart/crash paths cleanly
  - Before final generation, Rustyfin now compacts older unsummarized turns into persisted conversation memory when the prompt approaches the active context window, then rebuilds the prompt with that memory plus the newest raw turns instead of blindly overflowing the model context
  - Final answer generation now also carries forward a hidden structured block built from recent grounded follow-up contexts, so the model sees prior resolved entities and tool context even when older raw turns have been summarized away
  ...
  - The `/ai` response-mode dropdown now flows through the backend request path: `instant` biases toward shorter, faster final answers, `thinking` allows slower, more deliberate grounded responses, and `extended` uses the remaining context budget plus a 30-minute server-side generation ceiling for larger tasks such as document drafting, code writing, and self-review without exposing hidden chain-of-thought
  - AI context length is now chosen dynamically at model-load time from the GGUF model metadata, bounded by a host-memory heuristic and optional `RUSTFIN_AI_CONTEXT_LENGTH` override, and `/api/v1/ai/runtime` reports the effective loaded context window
  - Current grounded domains are account summary, visible calendar events/birthdays plus specific event details, confirmation-gated personal/shared calendar event creation, recurring birthday creation, calendar deletion with read-after-write verification, confirmation-gated markdown/plain-text document generation with authenticated download links plus verify/revise-before-save enforcement, recent visible channel activity, transcript-based summaries of accessible completed voice calls, authenticated downloads catalog entries, host-visible network topology and Rustyfin network settings, accessible libraries, library title search, recently added accessible library items, active public rooms, joinable rooms and invites, authenticated public weather via a fixed provider, admin-only host runtime stats, admin-only backup/service/transcode/storage/recent-error summaries, admin-only constrained public web search/page summary when enabled, and accessible Minecraft server status
  - Query understanding now supports calendar windows, deterministic `What is my next event?` prompts through `calendar_get_next_event`, broader next-up phrasing such as `What is the next thing coming up in my calendar?`, specific calendar-event detail prompts, named birthday lookups such as `When is Rachel's birthday?`, confirmation-gated delete prompts such as `Delete dentist appointment on 2026-06-09 from my calendar`, confirmation-gated document prompts such as `Create a markdown document summarizing my next event`, network questions such as interface/IP/hostname/remote-access prompts plus local-network connect prompts like `What IP should I use to connect to Rustyfin on my LAN?`, explicit current-time/current-date prompts for a named place such as `What time is it in Italy right now?` or `And the time in France?`, explicit AI runtime status prompts such as `What is the AI runtime status right now?`, recent channel-activity prompts, transcript-summary prompts such as asking what a transcribed call was about, room-mode filtering such as YouTube or screen-share rooms, joinable-room and invite prompts, recently-added library prompts, authenticated public-weather prompts such as `What is the temperature in Dublin right now?`, `Will it rain in Galway today?`, `What is the weather like this week for Campile in County Wexford?`, and recent-history prompts such as `Did it rain yesterday in Galway?`, admin-only host-runtime/service-health/storage/transcode/recent-error prompts, explicit public URLs, and Minecraft server status/name filtering such as online, offline, healthy, or a named server
  - Grounded `/ai` now keeps a typed per-user memory item store with bounded topical recall, so older relevant facts can be reinjected without replaying large raw histories
  - `/ai` now performs a CPU-friendly live operational retrieval pass over transcripts, recent errors, library items, and download artifacts before final generation, with ACL filtering kept server-side
  - Final `/ai` prompts now inject ranked compact grounding chunks with stable IDs and citation metadata rather than only raw tool payloads
  - Transcript grounding can now reuse exact excerpt windows with stable citation IDs, speaker labels, and offset metadata instead of only one static summary blob
  - `/ai` now surfaces compact source cards for grounded chunks in the chat UI so transcript/library/download/error evidence is inspectable after the answer
  - Current grounded domains are account summary, visible calendar events/birthdays plus specific event details, confirmation-gated personal/shared calendar event creation and recurring birthday creation with read-after-write verification, recent visible channel activity, transcript-based summaries of accessible completed voice calls, authenticated downloads catalog entries, host-visible network topology and Rustyfin network settings, accessible libraries, library title search, recently added accessible library items, active public rooms, joinable rooms and invites, authenticated public weather via a fixed provider, admin-only host runtime stats, admin-only backup/service/transcode/storage/recent-error summaries, admin-only constrained public web search/page summary when enabled, and accessible Minecraft server status
  - Query understanding now supports calendar windows, deterministic `What is my next event?` prompts through `calendar_get_next_event`, specific calendar-event detail prompts, named birthday lookups such as `When is Rachel's birthday?`, network questions such as interface/IP/hostname/remote-access prompts, recent channel-activity prompts, transcript-summary prompts such as asking what a transcribed call was about, room-mode filtering such as YouTube or screen-share rooms, joinable-room and invite prompts, recently-added library prompts, authenticated public-weather prompts such as `What is the temperature in Dublin right now?`, `Will it rain in Galway today?`, `What is the weather like this week for Campile in County Wexford?`, and recent-history prompts such as `Did it rain yesterday in Galway?`, admin-only host-runtime/service-health/storage/transcode/recent-error prompts, explicit public URLs, and Minecraft server status/name filtering such as online, offline, healthy, or a named server
  ...
  - `/api/v1/ai/chat` now returns deterministic server-authored answers for explicit current-date/current-time questions through `system_get_current_datetime`, including named-place local-time lookups resolved through public geocoding/timezone data, with bare country-name time queries such as `Italy` or `France` resolving to the country timezone instead of a same-named town or locality; recurring calendar-window replies now use the next in-window occurrence rather than the original stored birthday year, explicit AI runtime status prompts route to grounded AI runtime summaries instead of transcode stats, and geographic distance questions like `How far away is Italy from Spain?` must not fall through to datetime tools; it also returns deterministic next-event answers through grounded calendar ordering, concrete birthday detail lists from grounded birthday payloads, deterministic Rustyfin network connect guidance from grounded topology payloads, and deterministic joinable-room/invite summaries from grounded room payloads so LAN/IP/port and room/invite prompts do not drift into hallucinated locations, endpoints, or raw JSON dumps; local-network answers must prefer the real LAN interface over Docker bridges, virtual adapters, or Tailscale overlay addresses
  ...
  - Weather grounding now normalizes location phrases like `Campile in County Wexford`, carries recent weather follow-up context across a bare location reply, progressively reduces multi-part clarification follow-ups such as `Cork Muster Ireland` until a grounded match is found, supports recent history through `weather_get_history`, emits deterministic server-authored weather answers from grounded payloads instead of relying on model paraphrase alone, and does not treat date-only prompts like `next Tuesday` as weather requests
  - Bare same-name place queries such as `Galway` or `Dublin` now ask for clarification when the geocoder returns multiple plausible locations, instead of silently picking a U.S. locality; the same ambiguity handling also applies to named-place time queries
  ...
  - `/api/v1/ai/runtime` now also reports prompt-assembly telemetry such as loaded history turns, retained raw turns, summarized turns, prompt token estimate, and whether persisted conversation memory was used, so continuity issues can be inspected instead of guessed at
  ...
  - Admin `AI` now also exposes recent turn journals and compact boundaries so planner validation failures, compaction recovery, overloads, and document verification outcomes are inspectable without digging through the database
