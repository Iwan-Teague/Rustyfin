...
- AI
  ...
  - Current grounded domains are account summary, visible calendar events/birthdays plus specific event details, calendar conflict checks, calendar free-day lookups, next-free-day lookup, calendar busy-day counts, overlapping-event detection, next-event timing, confirmation-gated personal/shared calendar event creation and recurring birthday creation with read-after-write verification, calendar deletion with read-after-write verification, confirmation-gated AI conversation archive/delete/group-move actions scoped to the signed-in user with explicit title previews, confirmation-gated markdown/plain-text document generation with authenticated download links, recent visible channel activity, transcript-based summaries of accessible completed voice calls, authenticated downloads catalog entries plus artifact, checksum, install-step, and compatibility lookups, host-visible network topology, default-route, hostname-alias, DNS resolver, route table, active connections, interface counters, Wi-Fi status, VPN status, and Rustyfin network settings, accessible libraries with title search, library summaries, item summaries, item media/artwork/path details, duplicate-title scans, missing-metadata scans, and recently added accessible library items, recent stored memory facts, recent stored entities, recent stored memory changes, conflicting stored memory facts, exact stored entity lookups, stored entity provenance, stored entity relations, and bounded relation paths from the server-side memory graph, active public rooms, joinable rooms and invites, authenticated public weather via a fixed provider, authenticated AI runtime/model summary, admin-only host runtime stats, admin-only kernel/CPU/GPU/PCI/USB/temperature, boot-log, journal, process, listener, and disk-usage diagnostics plus backup/service/transcode/storage-path detail/mount detail/storage/recent-error summaries plus port-conflict summaries and port-conflict detail plus failed-unit summaries plus failed-unit detail, admin-only constrained public web search/page summary when enabled, and accessible Minecraft server status
  - Library search replies use compact server-authored summaries and grounded match lists instead of raw JSON payloads
  - Query understanding now supports calendar windows, deterministic `What is my next event?` prompts through `calendar_get_next_event`, broader next-up phrasing such as `What is the next thing coming up in my calendar?`, calendar conflict queries, calendar free-day queries, next-free-day prompts, calendar busy-day queries, overlapping-event prompts, calendar event-count queries, next-event timing prompts, specific calendar-event detail prompts, named/self/next birthday lookups such as `When is Rachel's birthday?`, `When is my birthday?`, or `What's the next birthday in my calendar?`, confirmation-gated delete prompts such as `Delete dentist appointment on 2026-06-09 from my calendar`, confirmation-gated document prompts such as `Create a markdown document summarizing my next event`, explicit grounded capability/tool-inventory prompts such as `What functions do you have access to in this environment?`, AI runtime/model questions such as `What AI model is loaded right now?` or `What backend are you using?`, network questions such as interface/IP/hostname/remote-access/default-route/hostname-alias prompts plus local-network connect prompts like `What IP should I use to connect to Rustyfin on my LAN?`, recent channel-activity prompts, transcript-summary prompts such as asking what a transcribed call was about, room-mode filtering such as YouTube or screen-share rooms, joinable-room and invite prompts, recently-added library prompts, library duplicate-title prompts such as `Do I have duplicate titles in my libraries?`, library missing-metadata prompts such as `What library items are missing metadata?`, library summary and item detail/media-path/artwork prompts, exact-memory entity prompts such as `Who is the exact Rachel in my family?`, recent-memory prompts such as `Who do you remember?`, recent-entity prompts such as `Show me recent people you remember`, memory-change prompts such as `What's new in my memory?`, conflict prompts such as `What conflicting facts do you have about Rachel?`, provenance prompts such as `Where did you learn about Rachel?`, entity-relation prompts such as `Who is Rachel related to?`, memory relation path prompts such as `How is Rachel related to Bob?`, authenticated public-weather prompts such as `What is the temperature in Dublin right now?`, `Will it rain in Galway today?`, `What is the weather like this week for Campile in County Wexford?`, and recent-history prompts such as `Did it rain yesterday in Galway?`, admin-only host-runtime/service-health/storage-path detail/mount detail/storage/transcode/recent-error/port-conflict/port-conflict detail/failed-unit/failed-unit detail prompts, explicit public URLs, and Minecraft server status/name filtering such as online, offline, healthy, or a named server
