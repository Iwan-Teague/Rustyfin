...
- AI
  ...
  - When a conversation would overrun the active context window, `/api/v1/ai/chat` now compacts older history into a model-generated memory note plus the newest raw turns before final generation, so long chats stay grounded in prior decisions instead of failing once prompt tokens crowd out the answer budget
