...
- Vault
  ...
  - The `/vault` Extension view now surfaces the exact browser-visible Rustyfin origin plus a copyable connection code so extension pairing can set the right server URL without guessing
  - The browser extension fill/save path is frame-aware for supported same-origin login iframes, while cross-origin iframe fill remains blocked by policy
  - The browser extension popup now keeps short-term server/pairing drafts when the popup closes, verifies the entered Rustyfin server URL against `/runtime-config`, and requests host access for that exact server before save/pair actions succeed
  - The browser extension requires a browser-trusted HTTPS certificate for the configured Rustyfin origin; a self-signed or otherwise untrusted local edge certificate will block extension networking until that certificate is trusted or the host is moved to a real trusted HTTPS origin
...
- Dictionary
  - Web `/dictionary` Human Dictionary surface for storing structured information about people in the user's life
  - Backed by PostgreSQL graph-shaped tables for spaces, workspaces, people, relationships, facts, documents, memberships, tree nodes, and account links
  - The current UI is tree-oriented around family, friends, work, and custom workspaces instead of a free-form graph canvas
  - All Dictionary reads and writes are enforced server-side through workspace membership and write-role checks; the frontend does not own permission logic
  - Workspace sharing is owner-managed through `dictionary_workspace_member`, including owner/editor/viewer roles and last-owner protection
  - Existing same-space canonical people can be attached into another workspace tree, with shortcut placement used when the person is already visible there
  - Non-empty Dictionary search stays backend-mediated through the workspace search route instead of relying only on a preloaded client slice
...
- AI
  ...
  - The offline AI eval harness under `crates/server/src/ai_eval_harness` now layers a versioned rubric-calibration judge on top of the deterministic suite checks, with manifest-pinned `judge_version` and `rubric_version`, optional rubric verdicts in the report artifact, explicit human-review routing for low-confidence or disagreement cases, a separate pairwise comparison experiment mode that flips response order and routes unstable order-sensitive cases to `no_winner`, and a production-trace ingestion workflow that writes raw trace archives separately from curated redacted judge-case rows
  - CI/release enforcement for that judge now goes through `scripts/ci/judge_gates.sh`, with `smoke` mode for PR-style gating and `release` mode for pinned-manifest deterministic replay plus stable JSON/Markdown artifact publication under `.tmp/gates/`
  - Grounded `/ai` can now also read the Human Dictionary for relationship-relative questions after the current Rustyfin account is linked to a Dictionary person
  ...
  - Current grounded domains are account summary, visible calendar events/birthdays plus specific event details, calendar conflict checks, calendar free-day lookups, next-free-day lookup, calendar busy-day counts, overlapping-event detection, next-event timing, confirmation-gated personal/shared calendar event creation and recurring birthday creation with read-after-write verification, calendar deletion with read-after-write verification, confirmation-gated AI conversation archive/delete/group-move actions scoped to the signed-in user with explicit title previews, confirmation-gated markdown/plain-text document generation with authenticated download links, recent visible channel activity, transcript-based summaries of accessible completed voice calls, authenticated downloads catalog entries plus artifact, source, release-note, checksum, install-step, and compatibility lookups, host-visible network topology, interface-by-IP, default-route, hostname-alias, DNS resolver, route table, active connections, interface counters, Wi-Fi status, VPN status, and Rustyfin network settings, accessible libraries with title search, library summaries, item summaries, item media/artwork/path details, source-path lookups, duplicate-title scans, missing-metadata scans, and recently added accessible library items, recent stored memory facts, recent stored entities, recent stored memory changes, conflicting stored memory facts, exact stored entity lookups, grounded person/profile summaries, stored entity provenance, stored entity relations, and bounded relation paths from the server-side memory graph, active public rooms, joinable rooms and invites, authenticated public weather via a fixed provider including exact-day forecast, hourly window, and recent-history lookups, authenticated AI runtime/model summary, admin-only host runtime stats, admin-only kernel/CPU/GPU/PCI/USB/temperature, boot-log, journal, process, listener, and disk-usage diagnostics plus backup/service/transcode/storage-path detail/mount detail/storage/recent-error summaries plus port-conflict summaries and port-conflict detail plus failed-unit summaries plus failed-unit detail, admin-only curated public web source catalog plus category-aware technology/business/economics search/page-summary lookups when enabled, and accessible Minecraft server status
  - Human Dictionary reads now support account identity lookup, visible workspace listing, visible people browse/search within a workspace, person bundle fetches, and relationship-relative reads such as birthdays, hobbies, and coworker/family listings from linked account context
  - Library search replies use compact server-authored summaries and grounded match lists instead of raw JSON payloads
  - Query understanding now supports calendar windows, deterministic `What is my next event?` prompts through `calendar_get_next_event`, broader next-up phrasing such as `What is the next thing coming up in my calendar?`, calendar conflict queries, calendar free-day queries, next-free-day prompts, calendar busy-day queries, overlapping-event prompts, calendar event-count prompts, next-event timing prompts, specific calendar-event detail prompts, named/self/next birthday lookups such as `When is Rachel's birthday?`, `When is my birthday?`, or `What's the next birthday in my calendar?`, confirmation-gated delete prompts such as `Delete dentist appointment on 2026-06-09 from my calendar`, confirmation-gated document prompts such as `Create a markdown document summarizing my next event`, explicit grounded capability/tool-inventory prompts such as `What functions do you have access to in this environment?`, AI runtime/model questions such as `What AI model is loaded right now?` or `What backend are you using?`, network questions such as interface/IP/hostname/remote-access/default-route/hostname-alias prompts plus local-network connect prompts like `What IP should I use to connect to Rustyfin on my LAN?` or exact interface-owner prompts like `Which interface owns 192.168.0.36?`, recent channel-activity prompts, transcript-summary prompts such as asking what a transcribed call was about, room-mode filtering such as YouTube or screen-share rooms, joinable-room and invite prompts, recently-added library prompts, library duplicate-title prompts such as `Do I have duplicate titles in my libraries?`, library missing-metadata prompts such as `What library items are missing metadata?`, library summary and item detail/media-path/artwork/source-path prompts, exact-memory entity prompts such as `Who is the exact Rachel in my family?`, grounded person/profile summary prompts such as `Give me a summary of Rachel` or `Show me Rachel's profile`, recent-memory prompts such as `Who do you remember?`, recent-entity prompts such as `Show me recent people you remember`, memory-change prompts such as `What's new in my memory?`, conflict prompts such as `What conflicting facts do you have about Rachel?`, provenance prompts such as `Where did you learn about Rachel?`, entity-relation prompts such as `Who is Rachel related to?`, memory relation path prompts such as `How is Rachel related to Bob?`, authenticated public-weather prompts such as `What is the temperature in Dublin right now?`, `Will it rain in Galway today?`, `What is the weather like this week for Campile in County Wexford?`, `What will the weather be on next Tuesday in Campile, Ireland?`, `What will the hourly weather be on next Tuesday in Campile, Ireland?`, and recent-history prompts such as `Did it rain yesterday in Galway?`, exact-date weather alias/location prompts, curated public web catalog/source-list prompts, technology/business/economics search and page-summary prompts, admin-only host-runtime/service-health/storage-path detail/mount detail/storage/transcode/recent-error/port-conflict/port-conflict detail/failed-unit/failed-unit detail prompts, explicit public URLs, and Minecraft server status/name filtering such as online, offline, healthy, or a named server
  ...
  - Weather grounding now normalizes location phrases like `Campile in County Wexford`, carries recent weather follow-up context across a bare location reply, supports exact-date forecast/history routing through `weather_get_forecast_for_date` and `weather_get_recent_history_for_date`, resolves location aliases through `weather_resolve_location_alias`, and emits deterministic server-authored weather answers from grounded payloads instead of relying on model paraphrase alone
  ...
  - The curated public web catalog is organized by technology, business, and economics source lists so the backend can route search and page-summary prompts to more reliable domains instead of generic browsing
  ...
  - Native installs now seed three starter GGUF models into the active AI model directory during setup: Qwen2.5 1.5B, Gemma 4 E2B, and Gemma 4 E4B. On a clean directory, Rustyfin downloads all three; if one of those starter files already exists, the installer backfills the missing starter models on the next install run; if the directory already contains only non-starter GGUFs, Rustyfin leaves them alone, so `/ai` is usable immediately after setup
...
- Channels
  ...
  - When TURN credentials are present, `/runtime-config` also appends TURN URLs for the host the browser is currently using, so public, LAN, and Tailscale access paths do not stay pinned to one private address family
...
- `/Users/iwanteague/Desktop/Rustyfin/ui/src/app/dictionary/page.tsx` - Human Dictionary page mounted by the host `/dictionary` route
- `/Users/iwanteague/Desktop/Rustyfin/ui/src/lib/dictionaryApi.ts` - Human Dictionary frontend API client
