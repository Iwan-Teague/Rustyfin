...
- An `AI` product area for the `/ai` assistant surface backed by `crates/ai-agent`
  ...
  - `system_get_current_datetime` is the canonical grounded tool for explicit current-date/current-time questions and relative-date anchoring; use the Rustyfin host local clock as the authority for host time, and resolve named-place local-time questions through backend-owned public geocoding/timezone lookup rather than model inference
