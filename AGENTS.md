...
- An `AI` product area for the `/ai` assistant surface backed by `crates/ai-agent`
  ...
  - when long conversations would crowd out the answer budget, compact older history into a backend-owned memory summary plus the newest raw turns before final generation so the assistant stays aware of prior decisions without overflowing the active context window
