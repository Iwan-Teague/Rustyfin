...
- A `Vault` product area for client-side encrypted password storage, web management, and browser-extension pairing/autofill
  ...
  - the `/vault` Extension view should surface the exact browser-visible Rustyfin origin and pairing guidance instead of making users guess which server URL to enter into the extension
...
- A `Dictionary` product area for Human Dictionary data about people, relationships, and account-linked family/friends/work graphs
  - the canonical host routes live under `/api/v1/dictionary`
  - current canonical persistence ownership is `crates/db/src/repo/dictionary.rs`
  - the canonical UI lives at `ui/src/app/dictionary/page.tsx` with its API client in `ui/src/lib/dictionaryApi.ts`
  - keep Dictionary permissions server-owned: workspace membership, write-role checks, and private/shared visibility boundaries must be enforced in Rust, not only in the UI
  - preserve the tree-oriented family/friends/work UX while keeping the backend graph-capable through people, tree nodes, relationships, facts, documents, memberships, and account links
  - workspace membership management stays owner-only, last-owner-safe, and scoped to `dictionary_workspace_member`; do not move member-management authority into the UI
  - same-space person reuse should go through the attach-existing-person path so shared canonical people can be mounted into another workspace tree without creating duplicate canonical rows
...
- An `AI` product area for the `/ai` assistant surface backed by `crates/ai-agent`
  ...
  - native installs should seed three starter GGUF models into the active AI model directory during setup: Qwen2.5 1.5B, Gemma 4 E2B, and Gemma 4 E4B. On a clean directory, Rustyfin should download all three; if one of those starter files already exists, the installer should backfill the missing starter models on the next install run; if the directory already contains only non-starter GGUFs, Rustyfin should leave them alone, so `/ai` is usable immediately after setup
  ...
  - current grounded `/ai` behavior is read-mostly and limited to account summary, visible calendar events/birthdays plus specific event details, calendar conflict checks, calendar free-day lookups, next-free-day lookup, calendar busy-day counts, overlapping-event detection, next-event timing, confirmation-gated personal/shared calendar event creation and recurring birthday creation, confirmation-gated archive/delete/group-move actions over the signed-in user's own AI conversations, Human Dictionary account identity lookups plus visible-workspace listing, visible-workspace people browse/search, and relationship-relative reads such as birthdays, hobbies, and coworker/family listings from the linked account graph, recent visible channel activity, transcript-based summaries of accessible completed voice calls, listing accessible voice-call transcripts the user can read, reading a specific accessible completed voice call's full saved transcript lines, authenticated downloads catalog entries plus artifact and package-detail lookups, authenticated AI runtime/model summary, host-visible network topology, interface-by-IP, default-route, hostname-alias, DNS resolver, route table, active connections, interface counters, Wi-Fi status, VPN status, and Rustyfin network settings, accessible libraries with title search, library summaries, item summaries, item media/artwork/path details, source-path lookups, recently added library items, recent stored memory facts, recent stored entities, recent stored memory changes, conflicting stored memory facts, exact stored entity lookups, grounded person/profile summaries, stored entity provenance, stored entity relations, bounded relation paths, active public rooms, joinable rooms and invites, authenticated public weather via a fixed provider including exact-date forecast, hourly window, and recent-history lookups, admin-only host runtime stats, admin-only kernel/CPU/GPU/PCI/USB/temperature, boot-log, journal, process, listener, and disk-usage diagnostics plus backup/service/transcode/storage-path detail/mount detail/storage/recent-error summaries plus port-conflict summaries and port-conflict detail plus failed-unit summaries plus failed-unit detail, admin-only curated public web source catalog plus category-aware technology/business/economics search/page-summary lookups when enabled, and accessible Minecraft server status
  ...
  - current query understanding also supports calendar window extraction, deterministic next-event prompts through `calendar_get_next_event`, broader next-up phrasing such as `What is the next thing coming up in my calendar?`, calendar conflict queries, calendar free-day queries, next-free-day prompts, calendar busy-day queries, overlapping-event prompts, calendar event-count prompts, next-event timing prompts, specific calendar-event detail prompts, named/self/next birthday lookups such as `When is Rachel's birthday?`, `When is my birthday?`, or `What's the next birthday in my calendar?`, confirmation-gated birthday or event-create prompts such as `Add Rachel's birthday on June 9, 2003`, confirmation-gated delete prompts such as `Delete dentist appointment on 2026-06-09 from my calendar`, confirmation-gated document prompts such as `Create a markdown document summarizing my next event`, AI runtime/model questions such as `What AI model is loaded right now?` or `What backend are you using?`, network questions such as interface/IP/hostname/remote-access/default-route/hostname-alias prompts plus LAN connect prompts like `What IP should I use to connect to Rustyfin on my local network?` or exact interface-owner prompts like `Which interface owns 192.168.0.36?`, recent channel-activity prompts, transcript-summary prompts for transcribed voice calls, download artifact detail prompts like `Tell me more about the RustyVault browser extension package`, download source and release-note prompts, room-mode filtering, joinable-room and invite prompts, recently added library prompts, library summary and item detail/media-path/artwork/source-path prompts, exact-memory entity prompts such as `Who is the exact Rachel in my family?`, grounded person/profile summary prompts such as `Give me a summary of Rachel` or `Show me Rachel's profile`, recent-memory prompts such as `Who do you remember?`, recent-entity prompts such as `Show me recent people you remember`, memory-change prompts such as `What's new in my memory?`, conflict prompts such as `What conflicting facts do you have about Rachel?`, provenance prompts such as `Where did you learn about Rachel?`, entity-relation prompts such as `Who is Rachel related to?`, memory relation path prompts such as `How is Rachel related to Bob?`, authenticated public-weather prompts such as current temperature, rain/forecast questions, county-style location phrasing, exact-date weather alias and recent-history prompts like yesterday for a named location, hour-by-hour weather on one exact day, curated public web catalog/source-list prompts, technology/business/economics search and page-summary prompts, admin-only host-runtime/service-health/storage-path detail/mount detail/storage/transcode/recent-error/port-conflict/port-conflict detail/failed-unit/failed-unit detail prompts, explicit public URLs, Minecraft server availability filtering, and named-server matching; keep new capability docs aligned to actual behavior
  ...
  - grounded weather behavior must stay backend-owned and deterministic: normalize location follow-ups like a bare `Campile, County Wexford, Ireland`, route current vs forecast vs exact-date forecast vs recent-history questions to the correct tool, resolve location aliases explicitly, and prefer server-authored grounded weather answers over model-only refusals when weather payloads already succeeded
  ...
  - the curated public web catalog groups technology, business, and economics sources so the backend can route search and page-summary prompts to more reliable domains without widening generic browsing
...
- `start-native.sh` is the supported production and development runtime path:
  ...
  - native runtime defaults to the `release` build profile; set `RUSTFIN_RUST_BUILD_PROFILE=dev` only when you intentionally need debug artifacts
