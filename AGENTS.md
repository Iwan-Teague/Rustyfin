...
- An `AI` product area for the `/ai` assistant surface backed by `crates/ai-agent`
  ...
  - `/api/v1/ai/runtime` should also expose prompt-assembly telemetry like loaded history turns, retained raw turns, summarized turns, prompt token estimate, recent grounded context count, and whether persisted conversation memory was used, so continuity regressions are observable
