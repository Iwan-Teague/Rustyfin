...
- An `AI` product area for the `/ai` assistant surface backed by `crates/ai-agent`
  ...
  - current grounded `/ai` behavior is read-only and limited to account summary, visible calendar events/birthdays plus specific event details, recent visible channel activity, transcript-based summaries of accessible completed voice calls, authenticated downloads catalog entries, host-visible network topology and Rustyfin network settings, accessible libraries, library title search, recently added library items, active public rooms, joinable rooms and invites, authenticated public weather via a fixed provider, admin-only host runtime stats, admin-only backup/service/transcode/storage/recent-error summaries, admin-only constrained public web search/page summary when enabled, and accessible Minecraft server status
  ...
  - persisted assistant turns must retain `activity_trace_json`, `stats_json`, grounding sources, and follow-up contexts so reloaded chats can replay the same assistant activity stack cleanly
  ...
  - unsupported create/edit/delete prompts must return server-authored refusals until explicit write-capable tools exist; do not let the model imply product writes succeeded
