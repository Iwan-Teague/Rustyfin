...
- An `AI` product area for the `/ai` assistant surface backed by `crates/ai-agent`
  ...
  - the `/ai` `Instant` and `Thinking` toggle must be enforced server-side: `instant` should bias toward shorter, faster final answers, while `thinking` should allow slower, more deliberate grounded responses without exposing hidden chain-of-thought
