...
- An `AI` product area for the `/ai` assistant surface backed by `crates/ai-agent`
  ...
  - deterministic server-authored grounded replies must cover current date/time, next-event lookups, birthday lists, and Rustyfin local-network connect answers when those payloads are already available, instead of letting the model paraphrase them into vague or incorrect summaries
