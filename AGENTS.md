...
- An `AI` product area for the `/ai` assistant surface backed by `crates/ai-agent`
  ...
  - current grounded `/ai` behavior is read-mostly and limited to account summary, visible calendar events/birthdays plus specific event details, confirmation-gated personal/shared calendar event creation, recurring birthday creation, calendar deletion, confirmation-gated markdown/plain-text document generation with authenticated download links, recent visible channel activity, transcript-based summaries of accessible completed voice calls, authenticated downloads catalog entries, host-visible network topology and Rustyfin network settings, accessible libraries, library title search, recently added library items, active public rooms, joinable rooms and invites, authenticated public weather via a fixed provider, admin-only host runtime stats, admin-only backup/service/transcode/storage/recent-error summaries, admin-only constrained public web search/page summary when enabled, and accessible Minecraft server status
  ...
  - `/ai` conversation history now includes persisted `group_name` and `sort_order` metadata so folder-style grouping and manual thread ordering survive reloads; preserve those fields instead of deriving the rail purely from `updated_ts`
  - grounded `/api/v1/ai/chat` now uses a model-assisted structured planner with typed validation/repair and planner diagnostics, but the backend remains the authority for registry validation, role filtering, deterministic fallback, and entity-follow-up normalization
  - the `/ai` response-mode selector must be enforced server-side: `instant` should bias toward shorter, faster final answers, `thinking` should allow slower, more deliberate grounded responses, and `extended` should reserve the largest bounded response budget for substantial tasks like drafting documents, writing/reviewing code, and self-checking work, with a hard server-side time ceiling and no hidden chain-of-thought exposure
  - persisted `/ai` conversations must maintain a server-owned memory layer for older turns instead of relying only on raw transcript replay; preserve the compact summary plus durable facts, user preferences, open loops, and active topics that get carried forward between turns
  - accepted conversation-backed turns must be durably journaled before first-token generation, and long-thread compactions must write explicit compact-boundary records so recovery can rebuild from the latest memory checkpoint plus post-boundary tail instead of replaying the full raw history
  - before final answer generation, compact older unsummarized turns into persisted conversation memory when the prompt approaches the active context window, then rebuild the prompt with that memory plus the newest raw turns; do not blindly stuff every prior turn into the model
  - final answer generation should also carry forward recent structured grounded follow-up contexts as hidden continuity state so the model still sees resolved entities/tool scope even when older raw turns are summarized away
  - AI context length must be selected dynamically from GGUF model metadata plus host-memory heuristics, with `RUSTFIN_AI_CONTEXT_LENGTH` only as a bounded override; keep `/api/v1/ai/runtime` aligned with the effective loaded context window instead of assuming a fixed `4096`
  - current query understanding also supports calendar window extraction, deterministic next-event prompts through `calendar_get_next_event`, broader next-up phrasing such as `What is the next thing coming up in my calendar?`, specific calendar-event detail prompts, named birthday lookups such as `When is Rachel's birthday?`, confirmation-gated birthday or event-create prompts such as `Add Rachel's birthday on June 9, 2003`, confirmation-gated delete prompts such as `Delete dentist appointment on 2026-06-09 from my calendar`, confirmation-gated document prompts such as `Create a markdown document summarizing my next event`, network questions such as interface/IP/hostname/remote-access prompts plus LAN connect questions like `What IP should I use to connect to Rustyfin on my local network?`, explicit current-time/current-date prompts for a named place such as `What time is it in Italy right now?` and short follow-ups like `And the time in France?`, explicit AI runtime status prompts such as `What is the AI runtime status right now?`, recent channel-activity prompts, transcript-summary prompts for transcribed voice calls, room-mode filtering, joinable-room and invite prompts, recently added library prompts, authenticated public-weather prompts such as current temperature, rain/forecast questions, county-style location phrasing, and recent-history prompts like yesterday for a named location, admin-only host-runtime/service-health/storage/transcode/recent-error prompts, explicit public URLs, Minecraft server availability filtering, and named-server matching; keep new capability docs aligned with actual behavior
  ...
  - `system_get_current_datetime` is the canonical grounded tool for explicit current-date/current-time questions and relative-date anchoring; use the Rustyfin host local clock as the authority for host time, resolve named-place local-time questions through backend-owned public geocoding/timezone lookup rather than model inference, ensure bare country-name queries such as `Italy` or `France` resolve to the country timezone instead of a same-named town or locality, and never let geographic-distance prompts like `How far away is Italy from Spain?` fall through to datetime routing
  - deterministic server-authored grounded replies must cover current date/time, next-event lookups, birthday lists, joinable-room/invite summaries, and Rustyfin local-network connect answers when those payloads are already available, instead of letting the model paraphrase them into vague or incorrect summaries or raw JSON dumps; local-network answers must prefer the real LAN interface over Docker bridges, virtual adapters, or Tailscale overlay addresses
  - `/api/v1/ai/artifacts/{id}/download` is the authenticated AI artifact delivery surface for confirmation-gated markdown/plain-text document generation; keep those artifacts user-scoped, bounded, and rendered as chat download links instead of exposing arbitrary filesystem writes
  - grounded weather behavior must stay backend-owned and deterministic: normalize location follow-ups like a bare `Campile, County Wexford, Ireland`, progressively reduce typoed or over-specified clarification follow-ups like `Cork Muster Ireland` until a grounded match is found, route current vs forecast vs recent-history questions to the correct tool, prefer server-authored grounded weather answers over model-only refusals when weather payloads already succeeded, and never let date-only prompts or date-correction follow-ups fall through to weather tools
  - when public geocoding returns multiple plausible same-name places for a bare location like `Galway` or `Dublin`, ask the user to clarify instead of silently picking one; apply the same ambiguity handling to named-place time queries that depend on timezone lookup
  ...
  - grounded `/ai` memory now combines the existing compact conversation summary with typed per-user memory items (`user_preference`, `durable_fact`, `environment_fact`, `runbook`, `tool_gotcha`, `open_loop`) and bounded selector-based recall; keep that recall topical, explainable, and ACL-safe
  - grounded `/ai` operational retrieval now includes a CPU-friendly live search layer over transcripts, recent errors, library items, and download artifacts; preserve server-side ACL filtering and compact chunk injection instead of feeding large raw payloads into the model
  - transcript follow-up grounding should prefer exact excerpt chunks with stable citation IDs, offsets, and speaker labels when the user is asking about something discussed in a call
  - grounded chunks and citations are now part of the persisted `/ai` turn state and visible `/ai` source cards; keep IDs stable so follow-ups and admin diagnostics can refer back to the same evidence
  - current query understanding also supports calendar window extraction, deterministic next-event prompts through `calendar_get_next_event`, specific calendar-event detail prompts, named birthday lookups such as `When is Rachel's birthday?`, confirmation-gated birthday or event-create prompts such as `Add Rachel's birthday on June 9, 2003`, network questions such as interface/IP/hostname/remote-access prompts, recent channel-activity prompts, transcript-summary prompts for transcribed voice calls, room-mode filtering, joinable-room and invite prompts, recently added library prompts, authenticated public-weather prompts such as current temperature, rain/forecast questions, county-style location phrasing, and recent-history prompts like yesterday for a named location, admin-only host-runtime/service-health/storage/transcode/recent-error prompts, explicit public URLs, Minecraft server availability filtering, and named-server matching; keep new capability docs aligned with actual behavior
  - host-local assistant date understanding must resolve relative and natural-language calendar prompts like `next Tuesday`, `today`, `tomorrow`, and `7th of April` deterministically on the backend; do not leave those to model guessing
  ...
  - grounded host-runtime summaries should include accurate human-readable memory fields, explicit AI runtime state like phase/request count/queue depth/context length for prompts such as `What is the AI runtime status right now?`, and per-turn stats should preserve planner/tool/generation/queue/load/end-to-end timing so `/ai` does not mislabel generation time as total turn time
  - `/api/v1/ai/runtime` should also expose prompt-assembly telemetry like loaded history turns, retained raw turns, summarized turns, prompt token estimate, recent grounded context count, compact-boundary counts, and whether persisted conversation memory was used, so continuity regressions are observable
  - grounded `/ai` now records traceable server logs plus assistant chat/tool runtime counters, durable turn journals, compact boundaries, and recent requests for the Admin `AI` tab; preserve those diagnostics when extending the assistant
  ...
  - confirmation-gated calendar write tools, including delete, must not execute without a valid unexpired token and a server-side verification pass; do not let the model imply product writes succeeded before that verification completes
  - generated markdown/plain-text download artifacts must go through deterministic validation plus a bounded verify/revise pass before persistence, and failed verification must save nothing while still surfacing verification diagnostics to admin/runtime observers
  - unsupported create/edit/delete prompts outside the explicit calendar create/birthday/delete and confirmation-gated document-create flow must still return server-authored refusals
  - do not introduce additional write-capable assistant actions beyond the explicit calendar flows and confirmation-gated downloadable document generation without an explicit confirmation-token or protected-action design
