...
export default function RustyVaultPage() {
  ...
  const [auditEvents, setAuditEvents] = useState<RustyVaultAuditEventResponse[]>([]);
  ...
  async function reloadRustyVaultChrome() {
    ...
    const [nextConfig, nextPrefs, nextDevices, nextAudit] = await Promise.all([
      ...
      listRustyVaultAuditEvents(session.access_token).then((response) => response.events).catch(() => []),
    ]);
    ...
    <div className="space-y-7 animate-rise">
        ...
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="chip">{rustyVaultSession ? 'Web session active' : 'No web session'}</span>
            </div>
      ...
      {cryptoSupported === false && (
        <div className="notice-error rounded-xl px-4 py-3 text-sm">
          This browser does not expose the required Web Crypto primitives for Vault. Use a current Chromium or Firefox build with Argon2id Web Crypto support.
        </div>
      )}
          ...
          <div className="panel space-y-5 p-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <button
                ...
                onClick={() => runAction('Generated password copied.', async () => writeClipboardWithTimeout(generatedPassword, prefs.clipboard_clear_seconds))}
              >
                Copy
              </button>
            </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm muted">
                    Download the current vault extension package from Downloads, extract it locally, then pair it from this page.
                  </p>
                </div>
                <span className="chip">Moved to Downloads</span>
              </div>
              <div className="flex flex-wrap gap-3">
