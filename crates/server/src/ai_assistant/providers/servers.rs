...
pub struct ServersToolProvider;

impl ToolProvider for ServersToolProvider {
    fn provider_id(&self) -> &'static str {
        "servers"
    }

    fn register(&self, registry: &mut ToolRegistryBuilder) {
        registry.register_tool(self, AssistantToolName::ServersListMinecraftStatus);
        registry.register_tool(self, AssistantToolName::ServersGetMinecraftServerSummary);
    }
    ...
    fn execute<'a>(
        ...
        Box::pin(async move { tools::execute_servers_provider_tool(state, context, call).await })
    }
}
