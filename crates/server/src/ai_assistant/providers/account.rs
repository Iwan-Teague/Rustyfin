...
pub struct AccountToolProvider;

impl ToolProvider for AccountToolProvider {
    fn provider_id(&self) -> &'static str {
        "account"
    }

    fn register(&self, registry: &mut ToolRegistryBuilder) {
        registry.register_tool(self, AssistantToolName::AccountGetProfileSummary);
    }
    ...
    fn execute<'a>(
        ...
        Box::pin(async move { tools::execute_account_provider_tool(state, context, call).await })
    }
}
