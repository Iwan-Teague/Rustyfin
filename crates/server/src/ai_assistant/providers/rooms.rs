...
pub struct RoomsToolProvider;

impl ToolProvider for RoomsToolProvider {
    fn provider_id(&self) -> &'static str {
        "rooms"
    }

    fn register(&self, registry: &mut ToolRegistryBuilder) {
        registry.register_tool(self, AssistantToolName::RoomsListActive);
        registry.register_tool(self, AssistantToolName::RoomsListJoinable);
        registry.register_tool(self, AssistantToolName::RoomsGetRoomSummary);
    }
    ...
    fn execute<'a>(
        ...
        Box::pin(async move { tools::execute_rooms_provider_tool(state, context, call).await })
    }
}
