...
pub struct DocumentsToolProvider;

impl ToolProvider for DocumentsToolProvider {
    fn provider_id(&self) -> &'static str {
        "documents"
    }

    fn register(&self, registry: &mut ToolRegistryBuilder) {
        registry.register_tool(self, AssistantToolName::DocumentCreateDownload);
    }
    ...
    fn execute<'a>(
        ...
        Box::pin(async move { tools::execute_documents_provider_tool(state, context, call).await })
    }
}
