...
impl Default for VaultRateLimiters {
    fn default() -> Self {
        Self::new()
    }
}
...
impl VaultLimitedAction {
    ...
    fn limiter(self, limiters: &VaultRateLimiters) -> &RateLimiter {
        match self {
...
fn classify_vault_limited_action(method: &Method, path: &str) -> Option<VaultLimitedAction> {
    ...
    {
        return Some(VaultLimitedAction::DeviceSessionPair);
    }
    if method == Method::POST
        && (path == "/protected-actions/challenge"
            || path.ends_with("/vault/protected-actions/challenge"))
    {
        return Some(VaultLimitedAction::ProtectedActionChallenge);
    }
    if method == Method::POST && (path == "/lookup" || path.ends_with("/vault/lookup")) {
        return Some(VaultLimitedAction::Lookup);
    }

    None
}

fn extract_vault_rate_limit_identity(headers: &HeaderMap) -> String {
        ...
        .filter(|value| !value.trim().is_empty())
    {
        ...
        return format!("vault-access:{}", &hash[..VAULT_KEY_PREFIX_LEN]);
    }
...
fn apply_vault_api_security_headers(response: &mut Response) {
...
pub async fn vault_rate_limit_middleware(request: Request, next: Next) -> Response {
    let limiters = match request.extensions().get::<VaultRateLimiters>().cloned() {
        Some(limiters) => limiters,
        None => return next.run(request).await,
    };

    let action = classify_vault_limited_action(request.method(), request.uri().path());
    ...
    let key = format!(
        "vault:{}:{}",
        action.slug(),
        extract_vault_rate_limit_identity(request.headers())
    );
        ...
        Err(retry_after) => {
            let envelope = ErrorEnvelope {
                error: rustfin_core::error::ErrorBody {
                    ...
                    message: "too many vault requests".to_string(),
            ...
            apply_vault_api_security_headers(&mut response);
            response
        }
    }
}

pub async fn vault_response_headers_middleware(request: Request, next: Next) -> Response {
