...
CREATE INDEX IF NOT EXISTS idx_channel_transcript_entry_search_vector
    ON channel_transcript_entry
    USING GIN (to_tsvector('simple', COALESCE(text, '') || ' ' || COALESCE(username, '')));

CREATE INDEX IF NOT EXISTS idx_download_artifact_search_vector
    ON download_artifact
    USING GIN (
        to_tsvector(
            'simple',
            COALESCE(title, '') || ' ' || COALESCE(summary, '') || ' ' || COALESCE(detail, '') || ' ' || COALESCE(artifact_id, '')
        )
    );
...
CREATE INDEX IF NOT EXISTS idx_job_error_search_vector
    ON job
    USING GIN (
        to_tsvector(
            'simple',
            COALESCE(kind, '') || ' ' || COALESCE(status, '') || ' ' || COALESCE(error, '') || ' ' || COALESCE(payload_json, '')
        )
    );
