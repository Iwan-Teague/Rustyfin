...
pub async fn update_event(
    ...
    let result = sqlx::query(
        "UPDATE calendar_event \
         SET scope = ?, owner_user_id = ?, title = ?, description = ?, event_date = ?, \
             event_type = ?, recurrence = ?, birthday_year = ?, updated_ts = ? \
         WHERE id = ?",
    )
    .bind(&patch.scope)
...
pub async fn delete_event(pool: &SqlitePool, event_id: &str) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM calendar_event WHERE id = ?")
        .bind(event_id)
        .execute(pool)
        .await?;
    Ok(result.rows_affected() > 0)
    ...
    let rows: Vec<(
        ...
        "SELECT e.id, e.scope, e.owner_user_id, owner.username, e.title, e.description, \
         ...
         WHERE (
             e.scope = 'global'
             OR (? = 1)
             OR (e.scope = 'personal' AND e.owner_user_id = ?)
         ) \
           AND (
             (e.recurrence = 'none' AND e.event_date >= ? AND e.event_date <= ?)
