...
use rustfin_server::ai_assistant::types::AssistantHistoryMessage;
use rustfin_server::ai_assistant::{plan_tool_calls_with_model_assist, AssistantPlannerMode};
use rustfin_server::auth::AuthUser;
...
pub async fn run(fixtures_dir: &Path) -> Result<EvalSuiteReport> {
    ...
    for case in &cases {
        ...
        let violations = actual_tools
            .iter()
            .filter(|tool| case.forbidden_tools.iter().any(|forbidden| forbidden == *tool))
            .cloned()
            .collect::<Vec<_>>();
    ...
    metrics.insert("planner_repair_success_rate".to_string(), repair_success_rate);
    metrics.insert("planner_deterministic_fallback_rate".to_string(), fallback_rate);
