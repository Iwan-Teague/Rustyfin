...
use anyhow::Result;
use rustfin_server::ai_tasks::executor::run_task_with_store;
use rustfin_server::ai_tasks::store::{AiTaskStore, MemoryAiTaskStore};
use rustfin_server::ai_tasks::types::{AiTaskInput, AiTaskPhase, AiTaskStatus, CreateAiTaskRequest, TaskUserContext};
use rustfin_server::state::{AppState, RustyVaultRuntimeState};
...
pub async fn run(fixtures_dir: &Path) -> Result<EvalSuiteReport> {
    ...
    for case in &cases {
        ...
        let worker_split_match = if case.expected_worker_profiles.is_empty() {
            ...
            let profiles = checkpoints
                ...
                .map(|workers| {
                    workers
                        .iter()
                        .filter_map(|worker| worker.get("worker_profile").and_then(serde_json::Value::as_str))
                        .map(str::to_string)
                        .collect::<Vec<_>>()
                })
                .unwrap_or_default();
        ...
        if completed && artifact_content_match && runtime_ms <= case.allowed_runtime_budget_ms as u128 {
            completion_hits += 1;
        }
        ...
        details.push(TaskCaseResult {
            ...
            completed: completed && artifact_content_match && runtime_ms <= case.allowed_runtime_budget_ms as u128,
            artifact_generated,
            runtime_ms,
            worker_split_match,
            verification_seen,
    ...
    metrics.insert("task_completion_success_rate".to_string(), completion_metric);
    metrics.insert("task_verifier_correction_rate".to_string(), verification_metric);
    metrics.insert(
...
async fn cancellation_resume_correctness() -> bool {
    ...
    run_task_with_store(state, store.clone(), &task.id).await.is_ok()
        && store
            .get_task(&task.id)
            .await
            .ok()
...
fn test_state() -> AppState {
    ...
    let transcoder = std::sync::Arc::new(rustfin_transcoder::session::SessionManager::new(tc_config));
    ...
    AppState {
        ...
        engine: std::sync::Arc::new(tokio::sync::Mutex::new(rustfin_server::ai::EngineState::default())),
        transcoder,
        ffmpeg_path,
        ffprobe_path,
        transcoder_hw_accel: None,
        transcoder_hw_accel_required: false,
        cache_dir: std::env::temp_dir().join(format!("rustyfin-ai-evals-cache-{}", uuid::Uuid::new_v4())),
        watch_party_audio_dir: std::env::temp_dir().join(format!("rustyfin-ai-evals-audio-{}", uuid::Uuid::new_v4())),
        events: events_tx,
        watch_party: std::sync::Arc::new(rustfin_server::watch_party::manager::WatchPartyManager::new()),
        channel_manager: std::sync::Arc::new(rustfin_server::channels::manager::ChannelManager::new()),
    }
}
