...
use anyhow::Result;
use rustfin_server::ai_assistant::replies::{MAX_GROUNDING_PROMPT_CHARS, grounding_chunks_prompt, rank_and_compress_grounding_chunks};
use rustfin_server::ai_assistant::types::AssistantGroundingChunk;
...
pub fn run(fixtures_dir: &Path) -> Result<EvalSuiteReport> {
    ...
    for case in &cases {
        ...
        chunks.sort_by(|left, right| right.score.partial_cmp(&left.score).unwrap_or(Ordering::Equal));
    ...
    metrics.insert("retrieval_required_evidence_recall_at_5".to_string(), recall_metric);
    metrics.insert("retrieval_mrr".to_string(), mrr_metric);
    metrics.insert("retrieval_prompt_inclusion_correctness".to_string(), prompt_metric);
