type PageKind = 'login' | 'signup' | 'change_password' | 'username_step' | 'unknown';
...
type InlineState = {
  ...
  pendingAction: { kind: 'save_new' | 'update_existing' | 'add_uri'; message: string } | null;
...
let savePromptRoot: HTMLDivElement | null = null;
...
function safeSendMessage(payload: any): Promise<any> {
  try {
...
function fieldLabel(input: HTMLInputElement | HTMLTextAreaElement): string {
...
function fieldHint(input: HTMLInputElement | HTMLTextAreaElement): string {
  return [
    input.name,
    input.id,
    input.placeholder,
...
function firstByHint(inputs: Array<HTMLInputElement | HTMLTextAreaElement>, matcher: (hint: string) => boolean) {
...
function detectContext() {
  ...
  const usernameField =
    firstByHint(inputs, (hint) =>
      /user|email|login|identifier|account/.test(hint),
    ) ||
    inputs.find(
      (input) =>
  ...
  const currentPassword =
    firstByHint(passwordFields, (hint) => /current|old|existing/.test(hint)) ||
    null;
  const newPassword =
    firstByHint(passwordFields, (hint) => /new|create|choose/.test(hint)) ||
    passwordFields[passwordFields.length > 1 ? 1 : 0] ||
    null;
  const confirmPassword =
    firstByHint(passwordFields, (hint) => /confirm|repeat|verify/.test(hint)) ||
...
function positionFloating(element: HTMLElement, anchor: HTMLElement) {
...
async function showOverlayForField(field: HTMLInputElement | HTMLTextAreaElement) {
  activeField = field;
  ...
  const state = response as { ok: true } & InlineState;
  ...
  const mount = root.shadowRoot!.querySelector('.panel') as HTMLDivElement;
  mount.innerHTML = '';
  ...
  positionFloating(root, field as HTMLElement);
}

function fillField(input: HTMLInputElement | HTMLTextAreaElement | null, value: string) {
  if (!input) return;
  input.focus();
  input.value = value;
...
function createSavePrompt() {
  if (savePromptRoot) return savePromptRoot;
...
async function showSavePrompt(kind: 'save_new' | 'update_existing' | 'add_uri', message: string) {
  const root = createSavePrompt();
  const mount = root.shadowRoot!.querySelector('div') as HTMLDivElement;
...
function captureFormSubmission(form: HTMLFormElement) {
  ...
  const usernameValue =
    (context.usernameField instanceof HTMLInputElement || context.usernameField instanceof HTMLTextAreaElement)
      ...
      : '';
  const emailValue =
    context.usernameField instanceof HTMLInputElement && context.usernameField.type === 'email'
      ? context.usernameField.value
      : '';
  const passwordValue =
    context.pageKind === 'change_password'
