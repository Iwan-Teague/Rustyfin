...
function sameIdentity(summary: DecryptedRustyVaultSummary, draft: PendingItemDraft): boolean {
  ...
  return Boolean(
    (draftUser && draftUser === summaryUser) ||
...
function uriAlreadyPresent(item: MatchedVaultItem | null | undefined, url: string): boolean {
...
export function classifyPendingAction(args: {
  ...
  lastFilled: LastFilledContext | null;
  pageKind: PageKind;
}): PendingAction | null {
