import type { RustyVaultPreferences, RustyVaultSession } from './types.js';
...
function rustyvaultServerReachabilityMessage(serverBaseUrl: string): string {
...
export async function apiRequest<T>(path: string, options: RequestInit & { vaultAccessToken?: string } = {}): Promise<T> {
...
export async function refreshRustyVaultSession(): Promise<RustyVaultSession> {
  const current = await getSession();
  if (!current?.refresh_token) {
    throw new Error('Extension session is not paired');
  }
  const refreshed = await apiRequest<RustyVaultSession>('/vault/device-sessions/refresh', {
    method: 'POST',
...
export async function withRustyVaultSession<T>(path: string, options: RequestInit = {}): Promise<T> {
...
export async function getVaultPreferences(): Promise<RustyVaultPreferences> {
  const prefs = await withRustyVaultSession<Partial<RustyVaultPreferences>>('/vault/preferences');
  return {
