...
function normalizeArgon2Params(value) {
    return {
        memory_kib: typeof value?.memory_kib === 'number' && value.memory_kib > 0
            ? Math.floor(value.memory_kib)
            : DEFAULT_ARGON2_PARAMS.memory_kib,
        iterations: typeof value?.iterations === 'number' && value.iterations > 0
            ? Math.floor(value.iterations)
            : DEFAULT_ARGON2_PARAMS.iterations,
        parallelism: typeof value?.parallelism === 'number' && value.parallelism > 0
...
async function deriveWrapKey(masterMaterial) {
    return crypto.subtle.deriveKey({
        ...
        salt: EMPTY_SALT,
        info: WRAP_INFO,
    }, masterMaterial, { name: 'AES-GCM', length: 256 }, true, ['wrapKey', 'unwrapKey']);
}
async function deriveIndexKey(masterMaterial) {
    return crypto.subtle.deriveKey({
        ...
        salt: EMPTY_SALT,
        info: INDEX_INFO,
    }, masterMaterial, { name: 'HMAC', hash: 'SHA-256', length: 256 }, false, ['sign']);
}
export async function unlockRustyVault(masterPassword, userId, wrappedKey) {
    ...
    const vaultKey = await crypto.subtle.unwrapKey('raw', toArrayBuffer(fromHex(wrappedKey.wrapped_vault_key_hex)), wrapKey, {
        name: 'AES-GCM',
        iv: toArrayBuffer(fromHex(wrappedKey.wrap_nonce_hex)),
    }, { name: 'AES-GCM', length: 256 }, true, ['encrypt', 'decrypt', 'wrapKey']);
    return {
        user_id: userId,
...
export async function decryptRustyVaultSummary(unlocked, item) {
    ...
    }, unlocked.data_key, toArrayBuffer(fromHex(item.summary_ciphertext_hex)));
...
export async function decryptRustyVaultItem(unlocked, item) {
    ...
    }, unlocked.data_key, toArrayBuffer(fromHex(item.payload_ciphertext_hex)));
    return JSON.parse(decoder.decode(plaintext));
}
...
async function encryptBlob(unlocked, itemId, blobKind, version, value) {
    ...
    }, unlocked.data_key, encoder.encode(JSON.stringify(value)));
    return {
...
function filterAmbiguous(value, excludeAmbiguous) {
    if (!excludeAmbiguous)
        return value;
    return [...value].filter((char) => !AMBIGUOUS.has(char)).join('');
}
function nextAllowedChar(prefs, kindPreference) {
    const pools = [];
...
function applyWordCase(word, prefs) {
...
function replaceFirstMatch(source, replacements, prefs, kind) {
...
function buildMemorablePassword(prefs) {
...
export function generatePasswordFromPreferences(prefs) {
