...
export async function hash(params) {
    ...
    const encodedLen = module._argon2_encodedlen(tCost, mCost, parallelism, salt.length, hashLen, argon2Type);
    const encodedPtr = module.allocate(new Array(encodedLen + 1), 'i8', module.ALLOC_NORMAL);
    const version = 0x13;
    let err = null;
    let res = 0;
    try {
        res = module._argon2_hash_ext(tCost, mCost, parallelism, passwordPtr, password.length, saltPtr, salt.length, hashPtr, hashLen, encodedPtr, encodedLen, argon2Type, secretPtr, secretLen, adPtr, adLen, version);
    }
    catch (error) {
        err = error;
    }
