...
async function loadMatchesForUrl(tabId, url, override = {}) {
  ...
  const decrypted = await Promise.all(
    (lookup.items || []).map(async (item) => ({
      encrypted: item,
      summary: await decryptVaultSummary(unlocked, item),
    })),
  );
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    switch (message.type) {
      ...
      case 'unlock-vault': {
        const session = await refreshVaultSession().catch(() => getSession());
        if (!session?.access_token) {
          throw new Error('The extension is not paired to Rustyfin');
        }
        const config = await withVaultSession('/vault/config');
        if (!config?.active_wrapped_key) {
          throw new Error('No Rustyfin vault is configured for this account');
        }
        const claims = decodeJwtPayload(session.access_token);
        unlocked = await unlockVault(message.masterPassword, claims.sub, config.active_wrapped_key);
      ...
      case 'save-pending-item': {
        ...
        const encrypted = await encryptLoginItem(unlocked, item, settings.defaultMatchMode);
        await withVaultSession('/vault/items', {
          method: 'POST',
